...
/*
 * Function used to generate the insert statement used when combining the ICD9Data table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the ICD9Data data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.ICD9Data_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the ICD9Data data and insert it into a target table.
...
/*
 * Stored procedure that loops through all clients, captures the ICD9Data inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_ICD9Data AS
    ...
    DECLARE @sourceTable AS NVARCHAR(90) = N'TDMA_1Dim_ICD9Data';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(90) = N'xAnalytics_DW.dw.TEMP_TDMA_1Dim_ICD9Data';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(90) = N'xAnalytics_DW.dm.TDMA_1Dim_ICD9Data';
    ...
    -- Truncate the temp table for ICD9Data.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.ICD9Data_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_ICD9Data;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Dim_ICD9Data] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Dim_ICD9Data] b ON (a.ICD9Data_OID = b.ICD9Data_OID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    DECLARE @deleteUpdatedRecordsFromTarget AS NVARCHAR(4000) = N'DELETE ' + @targetTable + N' 
                                                                  ...
                                                                  INNER JOIN ' + @tempTable + N' b ON (a.ICD9Data_OID = b.ICD9Data_OID and a.SYS_SourceDB = b.SYS_SourceDB);'
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_ICD9Data;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_ICD9Data;
