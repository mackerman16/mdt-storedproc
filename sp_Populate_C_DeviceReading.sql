...
/*
 * Function used to generate the insert statement used when combining the C_DeviceReading table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the C_DeviceReading data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.C_DeviceReading_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the C_DeviceReading data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
                                                   N' SELECT TR_ID
      ,Biometric_DateEntered_Date_OID
      ,Member_OID
      ,Member_VID
      ,DeviceSetting_OID
      ,DeviceSetting_VID
      ,C_Biometric_OID
      ,C_Biometric_VID
      ,FK2_FK_Path2Instance_OID
      ,FK2_FK_Path2Instance_VID
      ,BiometricReadingID
      ,BiometricEntryTypeId
      ,BiometricEntryTypeName
      ,Biometric_DateEntered
      ,BiometricReading_1
      ,BiometricReading_2
      ,BiometricReading_3
      ,BiometricReading_4
      ...
      ,ReviewedBy
      ,EnterState
      ,DeviceLog_SYS_SourceDB
      ,DeviceLog_SYS_ETL_Timestamp
      ,DeviceLogID
      ,UnknownDeviceID
      ,DeviceID
      ,AnswerString
      ,DeviceUnknown_Acknowledged
      ,DeviceUnknown_SystemUserID
      ,DeviceUnknown_SYS_SourceDB
      ,DeviceUnknown_SYS_ETL_Timestamp
      ,PK_DeviceLogID
      ,MemberId
      ,ExcludedBySystemUserId
      ,C_DeviceReading_SystemUserId
      ,GIGO_Flag
      ,P2DLM_DeviceLogId
      ,P2DLM_MemberId
      ,P2DLM_TransmissionTime
      ,P2DLM_ProcessTime
      ,P2DLM_ProcessStatus
      ,P2DLM_GIGO_Flag
      ,P2DLM_SYS_SourceDB
      ,P2DLM_SYS_ETL_Timestamp
      ,P2IB_PathwayInstanceId
      ,P2IB_DeviceLogId
      ,P2IB_DefaultSettingGroupId
      ,P2IB_MemberSettingGroupId
      ,P2IB_GIGO_Flag
      ,P2IB_SYS_SourceDB
      ,P2IB_SYS_ETL_Timestamp
...
/*
 * Stored procedure that loops through all clients, captures the C_DeviceReading inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_C_DeviceReading AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Fct_C_DeviceReading';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Fct_C_DeviceReading';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Fct_C_DeviceReading';
    ...
    -- Truncate the temp table for C_DeviceReading.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.C_DeviceReading_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_C_DeviceReading;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Fct_C_DeviceReading] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Fct_C_DeviceReading] b ON (a.TR_ID = b.TR_ID and a.DeviceLog_SYS_SourceDB = b.DeviceLog_SYS_SourceDB);
    ...
    DECLARE @deleteUpdatedRecordsFromTarget AS NVARCHAR(4000) = N'DELETE ' + @targetTable + N' 
                                                                  ...
                                                                  INNER JOIN ' + @tempTable + N' b ON (a.TR_ID = b.TR_ID and a.DeviceLog_SYS_SourceDB = b.DeviceLog_SYS_SourceDB);'
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_C_DeviceReading;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_C_DeviceReading;
...
EXEC dw.sp_Populate_C_DeviceReading
