...
BEGIN
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
      ...
      ,CONCAT(SYS_SourceDB,' + ' - ' + 'TR_ID)
       FROM ' + @dbname + N'_DW.dm.TDMA_1Fct_AssessmentTransmitLog
...
CREATE PROCEDURE dw.sp_Populate_AssessmentTransmitLog AS
    ...
    DECLARE @tempTable AS NVARCHAR(100) = N'Analytics_DW.dw.TEMP_TDMA_1Fct_AssessmentTransmitLog';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'Analytics_DW.dm.TDMA_1Fct_AssessmentTransmitLog';
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM Analytics_DW.dm.TDMA_1Fct_AssessmentTransmitLog;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [Analytics_DW].[dm].[TDMA_1Fct_AssessmentTransmitLog] a 
                               INNER JOIN [Analytics_DW].[dw].[TEMP_TDMA_1Fct_AssessmentTransmitLog] b ON (a.TR_ID = b.TR_ID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM Analytics_DW.dm.TDMA_1Fct_AssessmentTransmitLog;
