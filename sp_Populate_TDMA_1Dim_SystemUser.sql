...
/*
 * Function used to generate the insert statement used when combining the SystemUser table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the SystemUser data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.SystemUser_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the SystemUser data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
      ...
      ,UserViewLevel
      ,UserView
      ,UserViewProfessional
      ,SYS_SourceDB
      ,SYS_ETL_Timestamp
      ,SystemUser_RUNNO_INSERT
      ,SystemUser_RUNNO_UPDATE
      ,CONCAT(SYS_SourceDB,' + ' - ' + 'SystemUser_OID)
      FROM ' + @dbname + N'_DW.dm.TDMA_1Dim_SystemUser
...
/*
 * Stored procedure that loops through all clients, captures the SystemUser inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_SystemUser AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Dim_SystemUser';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Dim_SystemUser';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Dim_SystemUser';
    ...
    -- Truncate the temp table for SystemUser.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                -- Checks how many records we should be capturing from SystemUser for each client and stores them in a temporary table.
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.SystemUser_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_SystemUser;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Dim_SystemUser] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Dim_SystemUser] b ON (a.SystemUser_OID = b.SystemUser_OID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    DECLARE @deleteUpdatedRecordsFromTarget AS NVARCHAR(4000) = N'DELETE ' + @targetTable + N' 
                                                                  ...
                                                                  INNER JOIN ' + @tempTable + N' b ON (a.SystemUser_OID = b.SystemUser_OID and a.SYS_SourceDB = b.SYS_SourceDB);'
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_SystemUser;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_SystemUser;
...
EXEC dw.sp_Populate_SystemUser
