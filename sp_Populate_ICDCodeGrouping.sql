...
/*
 * Function used to generate the insert statement used when combining the ICDCodeGrouping table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the ICDCodeGrouping data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.ICDCodeGrouping_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the ICDCodeGrouping data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
                                                   N' SELECT ICDCodeGrouping_OID
      ,ICDCodeGroupingId
      ,GroupingName
      ,GroupingDescription
      ,SYS_SourceDB
      ,SYS_ETL_Timestamp
      ,ICDCodeGrouping_RUNNO_INSERT
      ,ICDCodeGrouping_RUNNO_UPDATE
      ,CONCAT(SYS_SourceDB,' + ' - ' + 'ICDCodeGrouping_OID)
      FROM ' + @dbname + N'_DW.dm.TDMA_1Dim_ICDCodeGrouping
...
/*
 * Stored procedure that loops through all clients, captures the ICDCodeGrouping inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_ICDCodeGrouping AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Dim_ICDCodeGrouping';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Dim_ICDCodeGrouping';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Dim_ICDCodeGrouping';
    ...
    -- Truncate the temp table for ICDCodeGrouping.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                -- Checks how many records we should be capturing from ICDCodeGrouping for each client and stores them in a temporary table.
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.ICDCodeGrouping_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_ICDCodeGrouping;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Dim_ICDCodeGrouping] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Dim_ICDCodeGrouping] b ON (a.ICDCodeGrouping_OID = b.ICDCodeGrouping_OID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    DECLARE @deleteUpdatedRecordsFromTarget AS NVARCHAR(4000) = N'DELETE ' + @targetTable + N' 
                                                                  ...
                                                                  INNER JOIN ' + @tempTable + N' b ON (a.ICDCodeGrouping_OID = b.ICDCodeGrouping_OID and a.SYS_SourceDB = b.SYS_SourceDB);'
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_ICDCodeGrouping;
    ...
    DECLARE @insertRecordsFromTempToTarget AS NVARCHAR(4000) = N'INSERT INTO ' + @targetTable + 
      ...
      ,ICDCodeGrouping_RUNNO_INSERT
      ,ICDCodeGrouping_RUNNO_UPDATE
      ,SYS_SourceDB_ICDCodeGrouping_OID
       FROM ' + @tempTable + N';'
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_ICDCodeGrouping;
...
EXEC dw.sp_Populate_ICDCodeGrouping
