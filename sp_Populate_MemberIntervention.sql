...
/*
 * Function used to generate the insert statement used when combining the MemberIntervention table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the MemberIntervention data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.MemberIntervention_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the MemberIntervention data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
      ...
      ,Relationship_OID
      ,Relationship_VID
      ,InterventionDate_Date_OID
      ,FK_FK_Path2Instance_OID
      ,FK_FK_Path2Instance_VID
      ,MemberInterventionId
      ,InterventionTypeId
      ,InterventionTypeDesc
      ,InterventionTypeClass
      ,InterventionActionId
      ,InterventionActionDesc
      ,InterventionActionClass
      ,InterventionResultId
      ,InterventionResultDesc
      ,InterventionResultClass
      ,AlertLogId
      ,InterventionDate
      ,Complete
      ,Severity
      ...
      ,PathwayInstanceId
      ,P2PI_SYS_SourceDB
      ,P2PI_SYS_ETL_Timestamp
      ,PathwayInstanceFlag
      ,Member_Id
      ,MemberIntervention_RUNNO_INSERT
      ,CONCAT(SYS_SourceDB,' + ' - ' + 'TR_ID)
       FROM ' + @dbname + N'_DW.dm.TDMA_1Fct_MemberIntervention
...
/*
 * Stored procedure that loops through all clients, captures the MemberIntervention inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_MemberIntervention AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Fct_MemberIntervention';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Fct_MemberIntervention';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Fct_MemberIntervention';
    ...
    -- Truncate the temp table for MemberIntervention.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                -- Checks how many records we should be capturing from MemberIntervention for each client and stores them in a temporary table.
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.MemberIntervention_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_MemberIntervention;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Fct_MemberIntervention] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Fct_MemberIntervention] b ON (a.TR_ID = b.TR_ID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_MemberIntervention;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_MemberIntervention;
...
EXEC dw.sp_Populate_MemberIntervention
