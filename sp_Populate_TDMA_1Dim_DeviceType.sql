...
/*
 * Function used to generate the insert statement used when combining the DeviceType table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the DeviceType data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.DeviceType_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the DeviceType data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
      ...
      ,DeviceType_RUNNO_INSERT
      ,DeviceType_RUNNO_UPDATE
      ,CONCAT(SYS_SourceDB,' + ' - ' + 'DeviceType_VID)
      FROM ' + @dbname + N'_DW.dm.TDMA_1Dim_DeviceType
...
/*
 * Stored procedure that loops through all clients, captures the DeviceType inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_DeviceType AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Dim_DeviceType';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Dim_DeviceType';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Dim_DeviceType';
    ...
    -- Truncate the temp table for DeviceType.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                -- Checks how many records we should be capturing from DeviceType for each client and stores them in a temporary table.
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.DeviceType_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_DeviceType;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Dim_DeviceType] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Dim_DeviceType] b ON (a.DeviceType_VID = b.DeviceType_VID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    DECLARE @deleteUpdatedRecordsFromTarget AS NVARCHAR(4000) = N'DELETE ' + @targetTable + N' 
                                                                  ...
                                                                  INNER JOIN ' + @tempTable + N' b ON (a.DeviceType_VID = b.DeviceType_VID and a.SYS_SourceDB = b.SYS_SourceDB);'
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_DeviceType;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_DeviceType;
...
EXEC dw.sp_Populate_DeviceType
