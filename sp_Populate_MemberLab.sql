...
/*
 * Function used to generate the insert statement used when combining the MemberLab table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the MemberLab data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.MemberLab_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the MemberLab data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
      ...
      ,Member_VID
      ,MemberId
      ,MemberLab_RUNNO_INSERT
      ,CONCAT(SYS_SourceDB,' + ' - ' + 'TR_ID)
      FROM ' + @dbname + N'_DW.dm.TDMA_1Fct_MemberLab
...
/*
 * Stored procedure that loops through all clients, captures the MemberLab inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_MemberLab AS
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                -- Checks how many records we should be capturing from MemberLab for each client and stores them in a temporary table.
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.MemberLab_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
