...
/*
 * Function used to generate the insert statement used when combining the FlexMemberHealthCheck table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the FlexMemberHealthCheck data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.FlexMemberHealthCheck_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the FlexMemberHealthCheck data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
      ...
      ,FlexMemberHealthCheckId
      ,DaysInRotation
      ,RestartRotationOnDay
      ,StartDate
      ...
      ,GIGO_Flag
      ,MemberId
      ,FlexMemberHealthCheck_RUNNO_INSERT
      ,CONCAT(SYS_SourceDB,' + ' - ' + 'TR_ID)
       FROM ' + @dbname + N'_DW.dm.TDMA_1Fct_FlexMemberHealthCheck
...
/*
 * Stored procedure that loops through all clients, captures the FlexMemberHealthCheck inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_FlexMemberHealthCheck AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Fct_FlexMemberHealthCheck';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Fct_FlexMemberHealthCheck';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Fct_FlexMemberHealthCheck';
    ...
    -- Truncate the temp table for FlexMemberHealthCheck.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                -- Checks how many records we should be capturing from FlexMemberHealthCheck for each client and stores them in a temporary table.
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.FlexMemberHealthCheck_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_FlexMemberHealthCheck;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Fct_FlexMemberHealthCheck] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Fct_FlexMemberHealthCheck] b ON (a.TR_ID = b.TR_ID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_FlexMemberHealthCheck;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_FlexMemberHealthCheck;
...
EXEC dw.sp_Populate_FlexMemberHealthCheck
