...
/*
 * Function used to generate the insert statement used when combining the FlexAnswerType table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the FlexAnswerType data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.FlexAnswerType_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the FlexAnswerType data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
      ...
      ,[SYS_SourceDB]
      ,[SYS_ETL_Timestamp]
      ,[FlexAnswerType_RUNNO_INSERT]
      ,[FlexAnswerType_RUNNO_UPDATE]
      ,CONCAT(SYS_SourceDB,' + ' - ' + 'FlexAnswerType_OID)
      FROM ' + @dbname + N'_DW.dm.TDMA_1Dim_FlexAnswerType
...
/*
 * Stored procedure that loops through all clients, captures the FlexAnswerType inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_FlexAnswerType AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Dim_FlexAnswerType';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Dim_FlexAnswerType';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Dim_FlexAnswerType';
    ...
    -- Truncate the temp table for FlexAnswerType.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                -- Checks how many records we should be capturing from FlexAnswerType for each client and stores them in a temporary table.
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.FlexAnswerType_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_FlexAnswerType;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Dim_FlexAnswerType] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Dim_FlexAnswerType] b ON (a.FlexAnswerType_OID = b.FlexAnswerType_OID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    DECLARE @deleteUpdatedRecordsFromTarget AS NVARCHAR(4000) = N'DELETE ' + @targetTable + N' 
                                                                  ...
                                                                  INNER JOIN ' + @tempTable + N' b ON (a.FlexAnswerType_OID = b.FlexAnswerType_OID and a.SYS_SourceDB = b.SYS_SourceDB);'
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_FlexAnswerType;
    ...
    DECLARE @insertRecordsFromTempToTarget AS NVARCHAR(4000) = N'INSERT INTO ' + @targetTable + 
      ...
      ,[FlexAnswerType_RUNNO_INSERT]
      ,[FlexAnswerType_RUNNO_UPDATE]
      ,[SYS_SourceDB_FlexAnswerType_OID]
       FROM ' + @tempTable + N';'
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_FlexAnswerType;
...
EXEC dw.sp_Populate_FlexAnswerType
