...
/*
 * Function used to generate the insert statement used when combining the Path2InstanceBiometric table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the Path2InstanceBiometric data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.Path2InstanceBiometric_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the Path2InstanceBiometric data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
                                                   N' SELECT Path2InstanceBiometric_OID
      ,PathwayInstanceId
      ,DeviceLogId
      ,DefaultSettingGroupId
      ,MemberSettingGroupId
      ,GIGO_Flag
      ,SYS_SourceDB
      ,SYS_ETL_Timestamp
      ,Path2InstanceBiometric_RUNNO_INSERT
      ,Path2InstanceBiometric_RUNNO_UPDATE
      ,CONCAT(SYS_SourceDB,' + ' - ' + 'Path2InstanceBiometric_OID)
      FROM ' + @dbname + N'_DW.dm.TDMA_1Dim_Path2InstanceBiometric
...
/*
 * Stored procedure that loops through all clients, captures the Path2InstanceBiometric inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_Path2InstanceBiometric AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Dim_Path2InstanceBiometric';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Dim_Path2InstanceBiometric';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Dim_Path2InstanceBiometric';
    ...
    -- Truncate the temp table for Path2InstanceBiometric.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                -- Checks how many records we should be capturing from Path2InstanceBiometric for each client and stores them in a temporary table.
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.Path2InstanceBiometric_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_Path2InstanceBiometric;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Dim_Path2InstanceBiometric] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Dim_Path2InstanceBiometric] b ON (a.Path2InstanceBiometric_OID = b.Path2InstanceBiometric_OID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    DECLARE @deleteUpdatedRecordsFromTarget AS NVARCHAR(4000) = N'DELETE ' + @targetTable + N' 
                                                                  ...
                                                                  INNER JOIN ' + @tempTable + N' b ON (a.Path2InstanceBiometric_OID = b.Path2InstanceBiometric_OID and a.SYS_SourceDB = b.SYS_SourceDB);'
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_Path2InstanceBiometric;
    ...
    DECLARE @insertRecordsFromTempToTarget AS NVARCHAR(4000) = N'INSERT INTO ' + @targetTable + 
      ...
      ,Path2InstanceBiometric_RUNNO_INSERT
      ,Path2InstanceBiometric_RUNNO_UPDATE
      ,SYS_SourceDB_Path2InstanceBiometric_OID
       FROM ' + @tempTable + N';'
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_Path2InstanceBiometric;
...
EXEC dw.sp_Populate_Path2InstanceBiometric
