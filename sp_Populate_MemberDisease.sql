...
/*
 * Function used to generate the insert statement used when combining the MemberDisease table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the MemberDisease data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.MemberDisease_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the MemberDisease data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
                                                   N' SELECT TR_ID
      ,EnrollmentDate_Date_OID
      ,MemberDiseaseProgramEnrollmentId
      ,MemberDiseaseProgramId
      ,DiseaseProgramId
      ,ProgramName
      ,ProgramDescription
      ,DP_LastUpdated
      ,DP_SystemUserId
      ,VAVersion
      ,DiseaseProgram_SYS_SourceDB
      ,DiseaseProgram_SYS_ETL_Timestamp
      ,Ranking
      ,MDP_LastUpdated
      ,MDP_SystemUserId
      ,IncludeOnAssessment
      ,MDP_GIGO_Flag
      ,MDP_SYS_SourceDB
      ,MDP_SYS_ETL_Timestamp
      ,DisenrollReasonId
      ,DisenrollReason
      ,DisenrollReason_SYS_SourceDB
      ,DisenrollReason_SYS_ETL_Timestamp
      ,EnrollmentDate
      ,DisenrollmentDate
      ,MDPE_LastUpdated
      ,MDPE_SystemUserId
...
/*
 * Stored procedure that loops through all clients, captures the MemberDisease inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_MemberDisease AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Fct_MemberDisease';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Fct_MemberDisease';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Fct_MemberDisease';
    ...
    -- Truncate the temp table for MemberDisease.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.MemberDisease_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_MemberDisease;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Fct_MemberDisease] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Fct_MemberDisease] b ON (a.TR_ID = b.TR_ID and a.MDPE_SYS_SourceDB = b.MDPE_SYS_SourceDB);
    ...
    DECLARE @deleteUpdatedRecordsFromTarget AS NVARCHAR(4000) = N'DELETE ' + @targetTable + N' 
                                                                  ...
                                                                  INNER JOIN ' + @tempTable + N' b ON (a.TR_ID = b.TR_ID and a.MDPE_SYS_SourceDB = b.MDPE_SYS_SourceDB);'
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_MemberDisease;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_MemberDisease;
...
EXEC dw.sp_Populate_MemberDisease
