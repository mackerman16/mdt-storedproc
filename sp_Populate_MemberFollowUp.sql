...
/*
 * Function used to generate the insert statement used when combining the MemberFollowUp table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the MemberFollowUp data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.MemberFollowUp_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the MemberFollowUp data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
      ...
      ,FollowUpTypeId
      ,FollowUpType
      ,FollowUpType_SYS_SourceDB
      ,FollowUpType_SYS_ETL_Timestamp
      ,OwnerId
      ,DueDate
      ,Priority
      ,Complete
      ...
      ,SYS_ETL_Timestamp
      ,MemberFollowUpId
      ,Path2PathwayFollowUp_PathwayInstanceId
      ,Path2PathwayFollowUp_PathwayInstanceFlag
      ,MemberId
      ,MemberFollowUp_RUNNO_INSERT
      ,CONCAT(SYS_SourceDB,' + ' - ' + 'TR_ID)
      FROM ' + @dbname + N'_DW.dm.TDMA_1Fct_MemberFollowUp
...
/*
 * Stored procedure that loops through all clients, captures the MemberFollowUp inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_MemberFollowUp AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Fct_MemberFollowUp';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Fct_MemberFollowUp';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Fct_MemberFollowUp';
    ...
    -- Truncate the temp table for MemberFollowUp.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                -- Checks how many records we should be capturing from MemberFollowUp for each client and stores them in a temporary table.
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.MemberFollowUp_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_MemberFollowUp;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Fct_MemberFollowUp] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Fct_MemberFollowUp] b ON (a.TR_ID = b.TR_ID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_MemberFollowUp;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_MemberFollowUp;
...
EXEC dw.sp_Populate_MemberFollowUp
