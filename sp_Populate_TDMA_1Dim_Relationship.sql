...
/*
 * Function used to generate the insert statement used when combining the Relationship table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the Relationship data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.Relationship_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the Relationship data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
                                                   N' SELECT Relationship_VID
      ,Relationship_FromDate
      ,Relationship_ToDate
      ,Relationship_OID
      ,RelationshipId
      ,RelationshipTypeId
      ...
      ,Relationship_RUNNO_INSERT
      ,Relationship_RUNNO_UPDATE
      ,CONCAT(SYS_SourceDB,' + ' - ' + 'Relationship_VID)
      FROM ' + @dbname + N'_DW.dm.TDMA_1Dim_Relationship
...
/*
 * Stored procedure that loops through all clients, captures the Relationship inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_Relationship AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Dim_Relationship';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Dim_Relationship';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Dim_Relationship';
    ...
    -- Truncate the temp table for Relationship.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                -- Checks how many records we should be capturing from Relationship for each client and stores them in a temporary table.
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.Relationship_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_Relationship;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Dim_Relationship] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Dim_Relationship] b ON (a.Relationship_VID = b.Relationship_VID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    DECLARE @deleteUpdatedRecordsFromTarget AS NVARCHAR(4000) = N'DELETE ' + @targetTable + N' 
                                                                  ...
                                                                  INNER JOIN ' + @tempTable + N' b ON (a.Relationship_VID = b.Relationship_VID and a.SYS_SourceDB = b.SYS_SourceDB);'
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_Relationship;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_Relationship;
...
EXEC dw.sp_Populate_Relationship
