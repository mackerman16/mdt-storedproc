...
/*
 * Function used to generate the insert statement used when combining the MemberCommunication table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the MemberCommunication data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.MemberCommunication_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the MemberCommunication data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
      ...
      ,Relationship_OID
      ,Relationship_VID
      ,CommunicationDate_Date_OID
      ,FK_FK_Path2Instance_OID
      ,FK_FK_Path2Instance_VID
      ,MemberCommunicationId
      ,CommunicationTypeId
      ,CommunicationTypeDesc
      ,CommType_SYS_SourceDB
      ,CommType_SYS_ETL_Timestamp
      ,CommunicationReasonId
      ,CommunicationReasonDesc
      ,Verify
      ,IsPathway
      ,CommReason_SYS_SourceDB
      ,CommReason_SYS_ETL_Timestamp
      ,CommunicationResultId
      ,CommunicationResultDesc
      ,Contacted
      ,CommResult_CommunicationTypeID
      ,Enabled
      ,CourtesyCallReport
      ,LeftMessage
      ,NoAnswer
      ,NotAvailable
      ,Busy
      ,CommResult_SYS_SourceDB
      ,CommResult_SYS_ETL_Timestamp
      ,CommunicationDate
      ,LastUpdated
      ,SystemUserId
      ,MemberNoteId
...
/*
 * Stored procedure that loops through all clients, captures the MemberCommunication inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_MemberCommunication AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Fct_MemberCommunication';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Fct_MemberCommunication';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Fct_MemberCommunication';
    ...
    -- Truncate the temp table for MemberCommunication.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.MemberCommunication_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_MemberCommunication;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Fct_MemberCommunication] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Fct_MemberCommunication] b ON (a.TR_ID = b.TR_ID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_MemberCommunication;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_MemberCommunication;
...
EXEC dw.sp_Populate_MemberCommunication
