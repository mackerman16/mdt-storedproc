...
CREATE PROCEDURE dw.sp_Populate_Path2Instance AS
    ...
    DECLARE @tempTable AS NVARCHAR(100) = N'Analytics_DW.dw.TEMP_TDMA_1Dim_Path2Instance';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'Analytics_DW.dm.TDMA_1Dim_Path2Instance';
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM Analytics_DW.dm.TDMA_1Dim_Path2Instance;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [Analytics_DW].[dm].[TDMA_1Dim_Path2Instance] a 
                               INNER JOIN [Analytics_DW].[dw].[TEMP_TDMA_1Dim_Path2Instance] b ON (a.Path2Instance_VID = b.Path2Instance_VID and a.P2I_SYS_SourceDB = b.P2I_SYS_SourceDB);
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM Analytics_DW.dm.TDMA_1Dim_Path2Instance;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM Analytics_DW.dm.TDMA_1Dim_Path2Instance;
