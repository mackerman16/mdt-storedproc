...
/*
 * Function used to generate the insert statement used when combining the ZipCode table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the ZipCode data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.ZipCode_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the ZipCode data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
      ...
      ,SYS_SourceDB
      ,SYS_ETL_Timestamp
      ,ZipCode_RUNNO_INSERT
      ,ZipCode_RUNNO_UPDATE
      ,CONCAT(SYS_SourceDB,' + ' - ' + 'ZipCode_OID)
      FROM ' + @dbname + N'_DW.dm.TDMA_1Dim_ZipCode
...
/*
 * Stored procedure that loops through all clients, captures the ZipCode inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_ZipCode AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Dim_ZipCode';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Dim_ZipCode';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Dim_ZipCode';
    ...
    -- Truncate the temp table for ZipCode.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                -- Checks how many records we should be capturing from ZipCode for each client and stores them in a temporary table.
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.ZipCode_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_ZipCode;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Dim_ZipCode] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Dim_ZipCode] b ON (a.ZipCode_OID = b.ZipCode_OID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    DECLARE @deleteUpdatedRecordsFromTarget AS NVARCHAR(4000) = N'DELETE ' + @targetTable + N' 
                                                                  ...
                                                                  INNER JOIN ' + @tempTable + N' b ON (a.ZipCode_OID = b.ZipCode_OID and a.SYS_SourceDB = b.SYS_SourceDB);'
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_ZipCode;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_ZipCode;
