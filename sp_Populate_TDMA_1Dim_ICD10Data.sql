...
/*
 * Function used to generate the insert statement used when combining the ICD10Data table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the ICD10Data data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.ICD10Data_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the ICD10Data data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
      ...
      FROM ' + @dbname + N'_DW.dm.TDMA_1Dim_ICD10Data
...
/*
 * Stored procedure that loops through all clients, captures the ICD10Data inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_ICD10Data AS
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                -- Checks how many records we should be capturing from ICD10Data for each client and stores them in a temporary table.
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.ICD10Data_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    DECLARE @deleteUpdatedRecordsFromTarget AS NVARCHAR(4000) = N'DELETE ' + @targetTable + N' 
                                                                  ...
                                                                  INNER JOIN ' + @tempTable + N' b ON (a.ICD10Data_OID = b.ICD10Data_OID and a.SYS_SourceDB = b.SYS_SourceDB);'
