...
CREATE PROCEDURE dw.sp_Populate_MemberStatus AS
    ...
    DECLARE @tempTable AS NVARCHAR(100) = N'Analytics_DW.dw.TEMP_TDMA_1Fct_MemberStatus';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'Analytics_DW.dm.TDMA_1Fct_MemberStatus';
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM Analytics_DW.dm.TDMA_1Fct_MemberStatus;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [Analytics_DW].[dm].[TDMA_1Fct_MemberStatus] a 
                               INNER JOIN [Analytics_DW].[dw].[TEMP_TDMA_1Fct_MemberStatus] b ON (a.TR_ID = b.TR_ID and a.MemberStatus_SYS_SourceDB = b.MemberStatus_SYS_SourceDB);
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM Analytics_DW.dm.TDMA_1Fct_MemberStatus;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM Analytics_DW.dm.TDMA_1Fct_MemberStatus;
