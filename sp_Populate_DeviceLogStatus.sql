...
/*
 * Function used to generate the insert statement used when combining the DeviceLogStatus table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the DeviceLogStatus data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.DeviceLogStatus_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the DeviceLogStatus data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
                                                   N' SELECT DeviceLogStatus_OID
      ,DeviceLogStatusId
      ,DeviceLogMask
      ,DeviceLogStatus
      ,SYS_SourceDB
      ,SYS_ETL_Timestamp
      ,DeviceLogStatus_RUNNO_INSERT
      ,DeviceLogStatus_RUNNO_UPDATE
      ,CONCAT(SYS_SourceDB,' + ' - ' + 'DeviceLogStatus_OID)
      FROM ' + @dbname + N'_DW.dm.TDMA_1Dim_DeviceLogStatus
...
/*
 * Stored procedure that loops through all clients, captures the DeviceLogStatus inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_DeviceLogStatus AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Dim_DeviceLogStatus';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Dim_DeviceLogStatus';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Dim_DeviceLogStatus';
    ...
    -- Truncate the temp table for DeviceLogStatus.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                -- Checks how many records we should be capturing from DeviceLogStatus for each client and stores them in a temporary table.
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.DeviceLogStatus_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_DeviceLogStatus;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Dim_DeviceLogStatus] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Dim_DeviceLogStatus] b ON (a.DeviceLogStatus_OID = b.DeviceLogStatus_OID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    DECLARE @deleteUpdatedRecordsFromTarget AS NVARCHAR(4000) = N'DELETE ' + @targetTable + N' 
                                                                  ...
                                                                  INNER JOIN ' + @tempTable + N' b ON (a.DeviceLogStatus_OID = b.DeviceLogStatus_OID and a.SYS_SourceDB = b.SYS_SourceDB);'
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_DeviceLogStatus;
    ...
    DECLARE @insertRecordsFromTempToTarget AS NVARCHAR(4000) = N'INSERT INTO ' + @targetTable + 
      ...
      ,DeviceLogStatus_RUNNO_INSERT
      ,DeviceLogStatus_RUNNO_UPDATE
      ,SYS_SourceDB_DeviceLogStatus_OID
       FROM ' + @tempTable + N';'
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_DeviceLogStatus;
...
EXEC dw.sp_Populate_DeviceLogStatus
