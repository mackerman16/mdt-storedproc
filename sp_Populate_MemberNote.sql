...
/*
 * Function used to generate the insert statement used when combining the MemberNote table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the MemberNote data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.MemberNote_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the MemberNote data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
      ...
      ,Member_VID
      ,MemberId
      ,MemberNote_RUNNO_INSERT
      ,CONCAT(SYS_SourceDB,' + ' - ' + 'TR_ID)
      FROM ' + @dbname + N'_DW.dm.TDMA_1Fct_MemberNote
...
/*
 * Stored procedure that loops through all clients, captures the MemberNote inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_MemberNote AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Fct_MemberNote';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Fct_MemberNote';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Fct_MemberNote';
    ...
    -- Truncate the temp table for MemberNote.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                -- Checks how many records we should be capturing from MemberNote for each client and stores them in a temporary table.
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.MemberNote_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_MemberNote;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Fct_MemberNote] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Fct_MemberNote] b ON (a.TR_ID = b.TR_ID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_MemberNote;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Fct_MemberNote;
...
EXEC dw.sp_Populate_MemberNote
