...
/*
 * Function used to generate the insert statement used when combining the DeviceSetting table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the DeviceSetting data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.DeviceSetting_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the DeviceSetting data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
                                                   N' SELECT DeviceSetting_VID
      ,DeviceSetting_FromDate
      ,DeviceSetting_ToDate
      ,DeviceSetting_OID
      ,DeviceSettingID
      ,UnitID
      ,TargetWt
      ...
      ,InactiveDate
      ,WeightGainXLast
      ,AlertOnWeightGainXLast
      ,AlertOnMaxWeight
      ,AlertOnMinWeight
      ,AlertOnWeightGainXY1
      ,AlertOnWeightGainXY2
      ,AlertOnWeightLossXY1
      ,AlertOnSymptom
      ,AlertOnSymptomVariance
      ,AlertOnCompliance
      ,MemberWelcomePendingUpdate
      ,EDW
      ,DeviceUsesKilograms
      ,DeviceYesterdayCompare
      ,DeviceMaxCompare
      ,DisplayFluidInCups
      ,IsRequiredWeight
      ,WeightDailyRotationTypeId
      ,IsRequiredBloodPressure
      ,BloodPressureDailyRotationTypeId
      ,IsRequiredPulseOx
      ,PulseOxDailyRotationTypeId
      ,IsRequiredGlucose
      ,GlucoseDailyRotationTypeId
      ,IsRequiredPeakFlow
      ,PeakFlowDailyRotationTypeId
      ,BreakfastStart
      ,BreakfastEnd
      ,LunchStart
      ,LunchEnd
      ...
      ,TargetMaxGlucose
      ,TargetMinGlucose
      ,TargetMaxPreMealGlucose
      ,TargetMinPreMealGlucose
      ,TargetMaxAfterMealGlucose
      ,TargetMinAfterMealGlucose
      ,FluidLimitOunces
      ,SodiumLimit
      ,CholesterolMonths
      ,LDLGoal
      ,HDLGoal
      ,TriglycerideGoal
      ,TotalCholesterolGoal
      ,KetoneCheckLimit
      ,MaxPEFR
      ,MaxPainThreshold
      ,TargetHGBA1C
      ,DeviceDisplayFluidInCups
      ,DeviceDisplayWeightInKg
      ,DeviceDisplayTempInCelsius
      ,DeviceDisplayGlucoseInMillimoles
      ,EducationLevel
      ,VariableHexString
      ,ExpectedTransDaysInWeek
      ,NotReportedDays
      ,TargetSteps
      ,SYS_SourceDB
      ,SYS_ETL_Timestamp
...
/*
 * Stored procedure that loops through all clients, captures the DeviceSetting inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_DeviceSetting AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Dim_DeviceSetting';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Dim_DeviceSetting';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Dim_DeviceSetting';
    ...
    -- Truncate the temp table for DeviceSetting.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.DeviceSetting_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_DeviceSetting;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Dim_DeviceSetting] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Dim_DeviceSetting] b ON (a.DeviceSetting_VID = b.DeviceSetting_VID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    DECLARE @deleteUpdatedRecordsFromTarget AS NVARCHAR(4000) = N'DELETE ' + @targetTable + N' 
                                                                  ...
                                                                  INNER JOIN ' + @tempTable + N' b ON (a.DeviceSetting_VID = b.DeviceSetting_VID and a.SYS_SourceDB = b.SYS_SourceDB);'
    ...
    DECLARE @insertRecordsFromTempToTarget AS NVARCHAR(4000) = N'INSERT INTO ' + @targetTable + 
      ...
      ,DeviceSetting_RUNNO_INSERT
      ,DeviceSetting_RUNNO_UPDATE
      ,SYS_SourceDB_DeviceSetting_VID
       FROM ' + @tempTable + N';'
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_DeviceSetting;
...
EXEC dw.sp_Populate_DeviceSetting
