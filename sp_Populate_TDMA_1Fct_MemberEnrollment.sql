...
BEGIN
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
                                                   ...
                                                   FROM ' + @dbname + N'_DW.dm.TDMA_1Fct_MemberEnrollment
...
CREATE PROCEDURE dw.sp_Populate_MemberEnrollment AS
    ...
    DECLARE @tempTable AS NVARCHAR(100) = N'Analytics_DW.dw.TEMP_TDMA_1Fct_MemberEnrollment';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'Analytics_DW.dm.TDMA_1Fct_MemberEnrollment';
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM Analytics_DW.dm.TDMA_1Fct_MemberEnrollment;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [Analytics_DW].[dm].[TDMA_1Fct_MemberEnrollment] a 
                               INNER JOIN [Analytics_DW].[dw].[TEMP_TDMA_1Fct_MemberEnrollment] b ON (a.TR_ID = b.TR_ID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM Analytics_DW.dm.TDMA_1Fct_MemberEnrollment;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM Analytics_DW.dm.TDMA_1Fct_MemberEnrollment;
