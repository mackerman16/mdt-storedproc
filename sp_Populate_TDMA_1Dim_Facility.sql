...
CREATE PROCEDURE dw.sp_Populate_Facility AS
    ...
    DECLARE @tempTable AS NVARCHAR(100) = N'Analytics_DW.dw.TEMP_TDMA_1Dim_Facility';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'Analytics_DW.dm.TDMA_1Dim_Facility';
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM Analytics_DW.dm.TDMA_1Dim_Facility;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [Analytics_DW].[dm].[TDMA_1Dim_Facility] a 
                               INNER JOIN [Analytics_DW].[dw].[TEMP_TDMA_1Dim_Facility] b ON (a.Facility_VID = b.Facility_VID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM Analytics_DW.dm.TDMA_1Dim_Facility;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM Analytics_DW.dm.TDMA_1Dim_Facility;
