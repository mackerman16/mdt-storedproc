...
/*
 * Function used to generate the insert statement used when combining the OperationRole table
 ...
 *
 * Returns:
 *      @constructedInsert NVARCHAR(4000) - the insert statement used to capture the OperationRole data from a specific client and insert it into a target table
 */
CREATE FUNCTION dw.OperationRole_GenerateInsert (@table NVARCHAR(250), @dbname NVARCHAR(50), @lastRuntime DATETIME) RETURNS NVARCHAR(4000) AS
BEGIN

    -- The insert statement used to capture the OperationRole data and insert it into a target table.
    ...
    DECLARE @constructedInsert AS NVARCHAR(4000) = N'INSERT INTO ' + @table + 
                                                   N' SELECT OperationRole_OID
      ,OperationRoleID
      ,SYS_SourceDB
      ,SYS_ETL_Timestamp
      ,OperationID
      ,RoleID
      ,OperationRole_RUNNO_INSERT
      ,OperationRole_RUNNO_UPDATE
      ,CONCAT(SYS_SourceDB,' + ' - ' + 'OperationRole_OID)
      FROM ' + @dbname + N'_DW.dm.TDMA_1Dim_OperationRole
...
/*
 * Stored procedure that loops through all clients, captures the OperationRole inserts and updates
 * (since the last time this procedure was ran), and enters the data into one combined target table.
 */ 
CREATE PROCEDURE dw.sp_Populate_OperationRole AS
    ...
    DECLARE @sourceTable AS NVARCHAR(100) = N'TDMA_1Dim_OperationRole';

    -- Temporary table used during the process of moving data from client table to target table.
    DECLARE @tempTable AS NVARCHAR(100) = N'xAnalytics_DW.dw.TEMP_TDMA_1Dim_OperationRole';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'xAnalytics_DW.dm.TDMA_1Dim_OperationRole';
    ...
    -- Truncate the temp table for OperationRole.
    ...
    OPEN dbc
        ...
        BEGIN
            BEGIN TRY
                -- Checks how many records we should be capturing from OperationRole for each client and stores them in a temporary table.
                ...
                DECLARE @insert AS NVARCHAR(4000) = dw.OperationRole_GenerateInsert(@tempTable, @dbname, @lastRuntime)
                EXECUTE (@insert)
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_OperationRole;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [xAnalytics_DW].[dm].[TDMA_1Dim_OperationRole] a 
                               INNER JOIN [xAnalytics_DW].[dw].[TEMP_TDMA_1Dim_OperationRole] b ON (a.OperationRole_OID = b.OperationRole_OID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    DECLARE @deleteUpdatedRecordsFromTarget AS NVARCHAR(4000) = N'DELETE ' + @targetTable + N' 
                                                                  ...
                                                                  INNER JOIN ' + @tempTable + N' b ON (a.OperationRole_OID = b.OperationRole_OID and a.SYS_SourceDB = b.SYS_SourceDB);'
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_OperationRole;
    ...
    DECLARE @insertRecordsFromTempToTarget AS NVARCHAR(4000) = N'INSERT INTO ' + @targetTable + 
      ...
      ,OperationRole_RUNNO_INSERT
      ,OperationRole_RUNNO_UPDATE
      ,SYS_SourceDB_OperationRole_OID
       FROM ' + @tempTable + N';'
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM xAnalytics_DW.dm.TDMA_1Dim_OperationRole;
...
EXEC dw.sp_Populate_OperationRole
