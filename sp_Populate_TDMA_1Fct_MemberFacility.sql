...
CREATE PROCEDURE dw.sp_Populate_MemberFacility AS
    ...
    DECLARE @tempTable AS NVARCHAR(100) = N'Analytics_DW.dw.TEMP_TDMA_1Fct_MemberFacility';

    -- Target table.
    DECLARE @targetTable AS NVARCHAR(100) = N'Analytics_DW.dm.TDMA_1Fct_MemberFacility';
    ...
    SELECT @targetCountBeforeDeletes = COUNT(*) FROM Analytics_DW.dm.TDMA_1Fct_MemberFacility;
    ...
    SELECT @totalsourceUpdateCount = COUNT(*)
                               FROM [Analytics_DW].[dm].[TDMA_1Fct_MemberFacility] a 
                               INNER JOIN [Analytics_DW].[dw].[TEMP_TDMA_1Fct_MemberFacility] b ON (a.TR_ID = b.TR_ID and a.SYS_SourceDB = b.SYS_SourceDB);
    ...
    SELECT @targetCountBeforeAdditions = COUNT(*) FROM Analytics_DW.dm.TDMA_1Fct_MemberFacility;
    ...
    SELECT @targetCountAfterAdditions = COUNT(*) FROM Analytics_DW.dm.TDMA_1Fct_MemberFacility;
